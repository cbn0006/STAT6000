...
There is not much difference between all five approaches with Least Squares and PCR being the best and Lasso being the worst. We can somewhat accurately predict applicants because colleges usually receive ~50k applications, so that is off by roughly 2-3%.
...
All models don't perform well/reasonably well on this dataset because they all have roughly the same MSE and the predicted value is supposed to be somewhere around 0.03-0.006. This means that these models are off on a large magnitude
